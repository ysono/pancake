...
use crate::storage::engine_ssi::entryset::CommittedEntrySetInfo;
use crate::storage::engines_common::Entry;
use crate::storage::serde::{
    DatumReader, DatumWriter, Deser, KeyValueRangeIterator, OptDatum, ReadResult, Ser,
};
    ...
    OptDatum<V>: Ser,
{
    ...
    where
        ...
        V: 'a,
    {
        fs::create_dir(&*entryset_info.entryset_dir)?;
        let kv_file_path = entryset_info.entryset_dir.sstable_file_path();
        ...
        let commit_info_path = entryset_info.entryset_dir.commit_info_file_path();
        let commit_info_file = OpenOptions::new()
            .create_new(true)
            .write(true)
            .open(&commit_info_path)?;
        let mut commit_info_writer = BufWriter::new(commit_info_file);
        entryset_info.commit_info.ser(&mut commit_info_writer)?;
    ...
    OptDatum<V>: Deser,
{
        ...
        K: PartialOrd<Q>,
    {
        ...
        let kv_file_path = self.entryset_info.entryset_dir.sstable_file_path();
        let mut res_file_iter = File::open(&kv_file_path)
...
impl<K, V> SSTable<K, V> {
    pub fn is_empty(&self) -> Result<bool> {
        if !self.sparse_file_offsets.is_empty() {
            return Ok(false);
        }

        let kv_file_path = self.entryset_info.entryset_dir.sstable_file_path();
        let meta = fs::metadata(&kv_file_path)?;
        Ok(meta.len() == 0)
    }
